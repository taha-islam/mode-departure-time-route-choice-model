...
class transitFares(collections.MutableMapping):
	...
	def readFromFile(self, fareFile):
		with open(fareFile) as f:
			content = f.readlines()
		...
		for element in content:
			...
			if len(keyAndValue) != 2 or len(keyAndValue[0]) == 0 or len(keyAndValue[1]) == 0:
				# skip this line
				continue
			fare = keyAndValue[1].split(',')
			if len(fare) > 1:
				self[keyAndValue[0]] = (float(fare[0]), float(fare[1]))
			else:
				self[keyAndValue[0]] = (float(fare[0]), 0)
	
	def writeToFile(self, fareFile):
		'''
		variable naming: agency:boarding_fare,distance_fare
		'''
		with open(fareFile, 'w') as f:
			for key in self.keys():
				f.write(key + '=' + str(self[key][0]) + ',' + str(self[key][1]) + '\n')
